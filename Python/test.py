...
#! ------------- SVM ------------------
    ...
    def execKernel(kernel: str):
        svm = SVC(kernel=kernel, max_iter=10000)
        svm.fit(trainingX, trainingY)
        predictions = svm.predict(testX)
    ...
    joblib.Parallel(n_jobs=-1)(joblib.delayed(execKernel)(kernel) for kernel in kernels)
